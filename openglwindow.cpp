...
OpenGLWidget::OpenGLWidget(const Camera & cam, QWidget *parent)noexcept :
    ...
    m_norms = QVector<QVector3D>();

    qDebug() << "OpengGLWidget constructor has been called";
...
void OpenGLWidget::initializeGL()
{
    ...
    //    m_vao.release();
    //    m_program->release();
    //qDebug() << "initializeGL has been called";
}
void OpenGLWidget::resizeGL(int w, int h) {
    ...
    //qDebug() << "resizeGL has been called";
    ...
    if (m_modelUpdated == true) {
        ...
        m_program->bind();
        {
            ...
            if (m_drawWithIBO == false)
            {
                ...
                m_ibo.bind();
                glDrawElements(GL_TRIANGLES, m_nVertex, GL_UNSIGNED_INT,0);
                //qDebug() << "asdfasdfsadfsafsadfsad";
                m_ibo.release();
            }
            m_vao.release();
        }
...
void OpenGLWidget::updateModel(const QVector<QVector3D> &vertices, const QVector<QVector3D> & normals)
{
    ...
    m_program->bind();
    m_vbo.setUsagePattern(QOpenGLBuffer::DynamicDraw);
    int totalBytes = vertices.count() * 3 * sizeof(GLfloat) + m_norms.count() * 3 * sizeof(GLfloat);
    m_vbo.allocate(totalBytes);
...
void OpenGLWidget::setTriangleMesh(const Point3f *vertices, const Point3f *normals,int nVertex, const unsigned int *faceIndices, int nFaceIndex)
{
   // m_timer->stop();
    m_drawWithIBO = true;
    m_nVertex = nFaceIndex;
    if (isValid() == false)
        return;
    ...
    int normalTotalBytes = nVertex * sizeof(Float) * 3;
    int vertexAndNormalsTotalBytes = vertexTotalBytes + normalTotalBytes;

    int faceIndexTotalBytes = nFaceIndex * sizeof(unsigned int);
    int normalIndexTotalBytes = nVertex * sizeof(unsigned int);
...
//void OpenGLWidget::setTriangleMesh(const TriangleMesh & mesh)
//{
//    setTriangleMesh(mesh.getVerticesArray(), 
